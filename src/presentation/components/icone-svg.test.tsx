...
describe('IconeSvg', () => {
  it('renderiza um path por item do array recebido', async () => {
    const resultado = await render(<IconeSvg path={icones.adicionar} cor="#63B4D4" />);
  ...
  it('aplica a mesma cor recebida a todos os paths, e cores diferentes geram valores diferentes', async () => {
    const azulejo = await render(<IconeSvg path={icones.buscar} cor="#EA6247" />);
    ...
    expect(paths.every((elemento) => elemento.props.stroke === primeiraCor)).toBe(true);

    const critico = await render(<IconeSvg path={icones.buscar} cor="#63B4D4" />);
