...
description: Decision rules from Vaughn Vernon's Implementing Domain-Driven Design for modeling aggregates, entities, value objects, repositories, and domain events. Use this whenever designing or reviewing a new domain concept in `src/domain/` (produto, compra, movimento_estoque, or any new aggregate), naming domain terms, deciding what belongs on an aggregate root vs. an application service, or checking whether a business rule leaked into infrastructure/presentation — even if the user just says "como modelar isso" or "isso devia ser regra de domínio ou do caso de uso?". See reference.md for the full book-derived reference when doing a deep domain-modeling audit.
license: MIT (rules distilled from https://github.com/ciembor/agent-rules-books, mini version)
---
...
Este app não tem múltiplos Bounded Contexts (é um único contexto doméstico, sem casa compartilhada no MVP — ver ADR-03), então a lente mais útil aqui é: Aggregates (`Produto`, `Compra`, `MovimentoEstoque`), invariantes protegidas na raiz (ex: `movimento_estoque` append-only, `produto.quantidade_atual` só muda na mesma transação do `INSERT` em `movimento_estoque` — já documentado no `CLAUDE.md`), Value Objects para conceitos como quantidade em milésimos e dinheiro em centavos (`domain/shared/`), e Repositórios por agregado (`ProdutoRepository`, `MovimentoRepository`, `CompraRepository`), nunca uma `Repository` genérica. Use esta skill ao desenhar um agregado ou regra de invariante nova — não para introduzir conceitos de DDD (Bounded Context múltiplo, Event Sourcing) que o projeto não precisa.
