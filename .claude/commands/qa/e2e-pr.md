...
3. Se não existe, invoque o subagent `test-automator` via `/qa:test` pedindo um flow Maestro novo para essa capability — ele já conhece `.maestro/README.md` (appId `com.triasoftware.repor`, seletor por texto visível, nunca por índice, assertion no vocabulário da UI).
