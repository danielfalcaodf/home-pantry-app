---
name: "OPSX: Test"
description: Run the full automated test flow (TDD + QA) of the current implemented change - the mandatory gate before /opsx:archive
allowed-tools: Bash(openspec:*), Bash(npm:*), Bash(npx:*), Bash(maestro:*)
category: Workflow
tags: [workflow, testing, qa, experimental]
---
...
**Scope**: this command always refers to the **current active changes** of this repository —
...
**Input**: Optionally specify a change name after `/opsx:test`. If omitted, infer from
conversation context; if ambiguous, run `openspec list --json` and use **AskUserQuestion**
(prefer changes with all/most tasks checked — this gate runs after implementation).
...
5. **Report the verdict**
   ...
   - **Any failure** → the change is NOT ready: list the failures, confirm the `/opsx:update`
     registration, and state that `/opsx:archive` is blocked until this command passes 100%.
