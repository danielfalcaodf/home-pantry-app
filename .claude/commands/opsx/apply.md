...
1. **Select the change**
   ...
   This command always operates on a **current active change** (never an archived one). Check
...
All tasks complete! Run `/opsx:test` to validate the full test flow — archiving with
`/opsx:archive` is only allowed after it passes 100%.
```
